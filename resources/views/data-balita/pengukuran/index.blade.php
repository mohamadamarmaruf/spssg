          ...
          <div class="col-md-12">
            ...
            <div class="card" style="border: none">
              <div class="text-right">
                <a href="{{ url('data-balita/pengukuran/add') }}/{{ $id }}"
                  ...
                  <tbody>
                    @foreach ($rs_ukur as $data)
                      <tr>
                        ...
                        <td>{{ $data->nik }} </td>
                        <td>{{ \Carbon\Carbon::parse($data->tgl_ukur)->locale('id_ID')->translatedFormat('d F Y') }} </td>
                        <td>{{ $data->stunting }} </td>
                        <td class="text-center">
                          <a href="{{ url('data-balita/pengukuran/detail') }}/{{ $data->id }}"
                            class="btn btn-outline-success btn-sm p-0 pl-1 pr-1">Detail</a>
                          @if ($role != 'user')
                            <a href="{{ url('data-balita/pengukuran/edit') }}/{{ $data->id }}"
                              class="btn btn-outline-warning btn-sm p-0 pl-1 pr-1">Edit</a>
                            <a href="{{ url('data-balita/pengukuran/delete') }}/{{ $data->id }}"
